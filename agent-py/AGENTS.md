...
All app-level code is in the `src/` directory. The entrypoint is `src/agent.py` (the Dockerfile depends on this). MongoDB-specific helpers are split into `src/db/` (client, index creation, seed scripts) and `src/tools/` (Voyage embeddings, agentic memory tools).

Be sure to maintain code formatting. You can use the ruff formatter/linter as needed: `uv run ruff format` and `uv run ruff check`.

## MongoDB Atlas integration

This agent integrates MongoDB Atlas across five hooks:

| Pattern | Hook | File |
| --- | --- | --- |
| Pre-load user context | `entrypoint` -> `preload_user` | `src/agent.py` |
| Function-tool CRUD | `@function_tool lookup_order` | `src/agent.py` |
| RAG via `$vectorSearch` | `on_user_turn_completed` | `src/agent.py` |
| Session report persistence | `on_session_end` | `src/agent.py` |
| Agentic memory tools | `@function_tool remember/recall/forget/search_memories` | `src/agent.py` + `src/tools/memory.py` |

Always use the PyMongo Async API (`from pymongo import AsyncMongoClient`). Note that `aggregate()` is a coroutine that returns an `AsyncCursor`; you must `await db.coll.aggregate(...)` and then `await cursor.to_list(...)`.
